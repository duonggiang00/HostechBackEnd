    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">Manage your payments and billing history</p>
        </div>
      </div>

      {/* Stats Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-indigo-600 rounded-[2rem] p-6 text-white relative overflow-hidden shadow-xl shadow-indigo-200">
        ...
        <div className="bg-white rounded-[2rem] p-6 border border-slate-200/60 shadow-sm relative overflow-hidden">
          <div className="relative z-10">
            <p className="text-slate-500 font-bold uppercase tracking-wider text-xs mb-2">Next Payment Due</p>
          ...
          <Clock className="absolute -bottom-6 -right-6 w-32 h-32 text-slate-100 -rotate-12" />
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between">
        <div className="relative flex-1 max-w-md group">
          ...
          <input 
            ...
            className="w-full pl-12 pr-4 py-3 bg-white border border-slate-200/60 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all font-medium text-slate-900 placeholder:text-slate-400"
          />
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <select 
              className="pl-10 pr-8 py-3 bg-white border border-slate-200/60 rounded-2xl text-sm font-bold text-slate-700 outline-none appearance-none focus:ring-2 focus:ring-indigo-500/20"
              value={filterStatus}
          ...
          <div className="grid grid-cols-1 divide-y divide-slate-100">
            {filteredInvoices.map((inv: any, idx: number) => (
              <motion.div 
                ...
                key={inv.id}
                className="p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:bg-slate-50 transition-colors group cursor-pointer"
              >
                <div className="flex items-center gap-6 flex-1 min-w-0">
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      <h4 className="text-base font-black text-slate-900 truncate">Invoice {inv.code}</h4>
                      <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider border ${getStatusColor(inv.status)}`}>
                ...
                <div className="flex items-center justify-between w-full md:w-auto gap-8">
                  <div className="text-left md:text-right">
                    ...
                    <p className="text-xl font-black text-slate-900">${inv.total.toLocaleString()}</p>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {['ISSUED', 'OVERDUE'].includes(inv.status) && (
                      <button className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 text-white rounded-xl text-sm font-bold hover:bg-slate-800 transition-colors shadow-lg shadow-slate-900/10 active:scale-95">
