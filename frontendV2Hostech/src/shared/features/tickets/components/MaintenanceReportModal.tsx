        ...
        <div className="fixed inset-0 z-100 flex items-end sm:items-center justify-center p-0 sm:p-6">
          ...
          >
            <div className="p-8 pb-4 flex items-center justify-between">
              <div>
                <span className="text-xs font-black uppercase text-slate-500 tracking-[0.3em]">Bước {step}/2</span>
                <h2 className="text-2xl font-black text-white uppercase tracking-tight mt-1">Báo sự cố</h2>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="p-8 pt-4 overflow-y-auto max-h-[70vh]">
              {step === 1 ? (
                <div className="space-y-6">
                  <p className="text-slate-400 font-medium">Bạn đang gặp vấn đề gì?</p>
                  <div className="grid grid-cols-2 gap-4">
                    {categories.map((cat) => (
                      ...
                      >
                        ...
                        <span className="text-sm font-black text-white uppercase tracking-wider block">{cat.label}</span>
                ...
                >
                  <div className="space-y-3">
                    <label className="text-xs font-black uppercase text-slate-500 tracking-widest pl-1">Mô tả chi tiết</label>
                    <textarea
                      autoFocus
                  ...
                  <div className="space-y-3">
                    <label className="text-xs font-black uppercase text-slate-500 tracking-widest pl-1">Hình ảnh đính kèm</label>
            ...
            <div className="p-8 pt-4 border-t border-white/5 flex gap-4">
              {step === 2 && (
                <button
                  onClick={() => setStep(1)}
                  className="px-6 py-4 rounded-2xl bg-white/5 text-slate-400 font-black uppercase tracking-wider hover:bg-white/10 transition-all"
                >
                  Quay lại
                </button>
              )}
              <button
                ...
                onClick={onClose}
                className="flex-1 px-8 py-4 bg-emerald-500 text-white font-black uppercase tracking-wider rounded-2xl hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg shadow-emerald-500/20 disabled:grayscale disabled:opacity-50"
              >
                {step === 1 ? 'Tiếp tục' : 'Gửi yêu cầu'}
              </button>
            </div>
          </motion.div>
        </div>
