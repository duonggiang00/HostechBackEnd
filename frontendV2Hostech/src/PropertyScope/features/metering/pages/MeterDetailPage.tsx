...
import { ArrowLeft, Loader2, Plus, Check, X, Clock, Trash2, Edit2, SendHorizontal, Eye, AlertTriangle } from 'lucide-react';
                            ...
                            <div className="flex gap-2 justify-end">
                              ...
                              {reading.status === 'SUBMITTED' && canApproveReadings && (
                                <>
                                  ...
                                  <button
                                    onClick={() => handleOpenRejectModal(reading.id)}
                              ...
                              {/* Re-submit for REJECTED readings */}
                              {reading.status === 'REJECTED' && canSubmitDraftReadings && (
                                <button
                                  onClick={() => handleSubmitReading(reading.id)}
                                  className="p-2 hover:bg-amber-100 dark:hover:bg-amber-500/20 rounded-lg transition-colors"
                                  title="Gửi lại"
                                >
                                  <SendHorizontal className="w-4 h-4 text-amber-600 dark:text-amber-400" />
                                </button>
                              )}

                              {/* Delete - Available for DRAFT/REJECTED readings */}
                              {(reading.status === 'DRAFT' || reading.status === 'REJECTED') && canManageReadings && (
                                <button
