      ...
      {roomId && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 bottom-0 w-full sm:w-[600px] lg:w-[800px] max-w-[100vw] bg-white shadow-2xl z-[70] overflow-y-auto custom-scrollbar border-l border-slate-200"
          >
            {isLoading ? (
              ...
              <div className="flex flex-col h-full items-center justify-center p-12 text-center">
                 <X className="w-16 h-16 text-rose-200 mb-6" />
                 <h3 className="text-xl font-black text-slate-900 mb-2">Lỗi tải phòng</h3>
                 <p className="text-slate-500 max-w-xs">{error?.message || 'Không thể lấy thông tin chi tiết cho đơn vị này.'}</p>
                 <button onClick={onClose} className="mt-8 px-8 py-3 bg-slate-900 text-white rounded-2xl font-bold">Đóng ngăn kéo</button>
              </div>
            ) : (
              <RoomDetailContent 
                room={room} 
