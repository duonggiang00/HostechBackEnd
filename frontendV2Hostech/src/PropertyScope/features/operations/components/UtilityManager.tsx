...
import { useMeters, useMeterReadings, type Meter } from '@/PropertyScope/features/metering/hooks/useMeters';
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        {roomMeters.slice(0, 4).map((meter) => (
          <div key={meter.id} className={`p-5 rounded-3xl text-white shadow-xl relative overflow-hidden group transition-all hover:scale-[1.02] ${
            meter.type === 'ELECTRIC' ? 'bg-gradient-to-br from-indigo-600 to-indigo-700 shadow-indigo-100' : 'bg-gradient-to-br from-cyan-500 to-cyan-600 shadow-cyan-100'
          }`}>
      ...
      <div className="flex items-center justify-between">
        ...
        <button 
          ...
          className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-2xl font-bold text-xs hover:bg-slate-800 transition-all shadow-lg shadow-slate-200 active:scale-95"
        >
      ...
      <div className="space-y-3">
        {readings.map((reading) => {
          ...
          return (
            <div key={reading.id} className="p-4 bg-white border border-slate-100 rounded-3xl flex items-center gap-4 hover:border-indigo-100 transition-all group">
              <div className={`p-2.5 rounded-2xl ${meter?.type === 'ELECTRIC' ? 'bg-indigo-50 text-indigo-600' : 'bg-cyan-50 text-cyan-600'}`}>
                {meter?.type === 'ELECTRIC' ? <Zap className="w-5 h-5" /> : <Droplets className="w-5 h-5" />}
              </div>
              
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-0.5">
                  <p className="font-black text-slate-900 text-sm">{reading.reading_value} {meter?.type === 'ELECTRIC' ? 'kWh' : 'm³'}</p>
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-lg border ${
                    reading.status === 'APPROVED' ? 'text-emerald-500 bg-emerald-50 border-emerald-100' : 'text-amber-500 bg-amber-50 border-amber-100'
                  }`}>
                    {reading.status}
                  </span>
                </div>
              ...
              <button 
                ...
                className="opacity-0 group-hover:opacity-100 p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all"
              >
              ...
              <div className="p-8 pb-4">
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-black text-slate-900 leading-none mb-1">OCR Meter Reading</h3>
                      <p className="text-xs text-slate-500 font-medium">Automatic extraction from photo</p>
                    </div>
                  </div>
                  <button onClick={() => setIsCapturing(false)} className="p-2 bg-slate-100 rounded-xl text-slate-500 hover:bg-slate-200">
                    <X className="w-5 h-5" />
                  </button>
                </div>

                <div className="grid grid-cols-2 gap-4 mb-8">
                  <div className="aspect-[4/3] bg-slate-100 rounded-3xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-slate-400 gap-2 overflow-hidden relative">
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex-1 p-5 bg-slate-50 rounded-3xl border border-slate-100 flex flex-col justify-center items-center text-center">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Detected Value</p>
                      <div className="relative group">
                        <h4 className="text-4xl font-black text-indigo-600 tracking-tighter">—</h4>
                        <div className="absolute -top-1 -right-4 animate-pulse">
                          <RefreshCcw className="w-4 h-4 text-indigo-300" />
                        </div>
                      </div>
                      <p className="text-[10px] text-slate-400 mt-2 font-bold uppercase">Confidence: 0%</p>
                    </div>
                  </div>
                </div>

                <div className="flex items-center gap-4 p-5 bg-amber-50 rounded-3xl border border-amber-100 mb-8">
                ...
                <div className="flex gap-4">
                  <button className="flex-1 py-4 bg-slate-100 text-slate-900 rounded-2xl font-black text-sm hover:bg-slate-200 transition-all active:scale-95">
                    Discard
                  </button>
                  <button className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 active:scale-95 flex items-center justify-center gap-2">
