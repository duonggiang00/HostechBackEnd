...
import type { RoomTemplate, ServiceTemplate, ContractTemplate, CreateRoomTemplatePayload, CreateServiceTemplatePayload, CreateContractTemplatePayload } from './types';
...
export const templatesApi = {
  ...
  // --- Contract Templates ---
  getContractTemplates: async (propertyId: string) => {
    const response = await apiClient.get(`/properties/${propertyId}/contract-templates`);
    return response.data?.data as ContractTemplate[];
  },
  
  getContractTemplate: async (propertyId: string, templateId: string) => {
    const response = await apiClient.get(`/properties/${propertyId}/contract-templates/${templateId}`);
    return response.data?.data as ContractTemplate;
  },
  
  createContractTemplate: async (propertyId: string, data: CreateContractTemplatePayload) => {
    const response = await apiClient.post(`/properties/${propertyId}/contract-templates`, data);
    return response.data?.data as ContractTemplate;
  },
  
  updateContractTemplate: async (propertyId: string, templateId: string, data: Partial<CreateContractTemplatePayload>) => {
    const response = await apiClient.put(`/properties/${propertyId}/contract-templates/${templateId}`, data);
    return response.data?.data as ContractTemplate;
  },

  deleteContractTemplate: async (propertyId: string, templateId: string) => {
    const response = await apiClient.delete(`/properties/${propertyId}/contract-templates/${templateId}`);
    return response.data;
  }
};
