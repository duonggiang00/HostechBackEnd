...
const DetailFloor = () => {
  ...
  const occupied = rooms.filter(
    (r) => r.status === "occupied" || r.status === "rented",
  ).length;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <span className="text-slate-700 flex items-center gap-2">
              ...
              {floor?.sort_order ?? floor?.order ?? floor?.sort_order ?? "—"}
            </span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-xs font-medium text-slate-400 uppercase tracking-wide">
            ...
            <span className="text-slate-700 flex items-center gap-2">
              ...
              {property?.name || (floor as any)?.property?.name || "—"}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="flex flex-col gap-1 p-3 bg-slate-50 rounded-xl border border-slate-100">
          ...
          <span className="text-sm text-slate-600 font-medium">
            {floor?.created_at
              ? new Date(floor.created_at).toLocaleDateString("vi-VN")
              : "—"}
          </span>
        </div>
        <div className="flex flex-col gap-1 p-3 bg-slate-50 rounded-xl border border-slate-100">
          <span className="text-xs text-slate-400 flex items-center gap-1">
            <Calendar size={11} /> Cập nhật
          </span>
          <span className="text-sm text-slate-600 font-medium">
            {floor?.updated_at
              ? new Date(floor.updated_at).toLocaleDateString("vi-VN")
              : "—"}
          </span>
        </div>
      </div>
    ...
    },
    {
      ...
      children: (
        <div className="animate-in fade-in duration-500 pt-2">
          <Rooms propertyId={propertyId} floorId={id} />
        </div>
      ),
    },
  ];
