...
export const updateProfile = async (data: Partial<ProfileType>): Promise<ProfileType> => {
...
export const getTwoFactorQrCode = async (): Promise<{ svg: string }> => {
    const res = await axiosClient.get("auth/user/two-factor-qr-code");
    return res.data;
};

export const enableTwoFactor = async (): Promise<void> => {
    await axiosClient.post("auth/user/two-factor-authentication");
};

export const disableTwoFactor = async (): Promise<void> => {
    await axiosClient.delete("auth/user/two-factor-authentication");
};

export const confirmTwoFactor = async (code: string): Promise<void> => {
    await axiosClient.post("auth/user/confirmed-two-factor-authentication", { code });
};

export const getTwoFactorRecoveryCodes = async (): Promise<string[]> => {
    const res = await axiosClient.get("auth/user/two-factor-recovery-codes");
    return res.data;
};

export const regenerateRecoveryCodes = async (): Promise<void> => {
    await axiosClient.post("auth/user/two-factor-recovery-codes");
};
