...
export const changePassword = async (data: PasswordDataType): Promise<void> => {
    await axiosClient.post("user/password", data); // Wait, Fortify usually uses user/password or profile/password. 
    // Let's use the one from the original build error or useProfile.ts context if available.
    // Actually, I'll use "user/password" as it's common in Fortify, but I should double check. 
    // Re-checking original ProfileApi (Step 816) it was "profile/change-password".
