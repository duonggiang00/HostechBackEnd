...
5. [ ] Tích hợp Route mới vào `src/app/routes/registry.ts` (thêm vào `manageRoutes` hoặc `portalRoutes`).
6. [ ] Đảm bảo gán route vào Layout phù hợp và có hiển thị trên Sidebar menu.

---

## 5. Lộ trình Kiến trúc Routing (Route Architecture Roadmap)

### Phase D1 — Hiện tại ✅
Sử dụng **Semantic Path** `/manage` cho Management Space (Owner/Manager/Staff).
- `/manage/*` → `LayoutManage` (trước đây là `LayoutAdmin`) — Dashboard, bảng quản lý
- `/me/*` → `LayoutPortal` — Giao diện Tenant (đang phát triển)
- `/admin/*` → Alias redirect → `/manage` (backward compatibility)

```
App Router:
├── /auth          → AuthPage (Login, OTP)
├── /manage/*      → ProtectedRoute → LayoutManage
│   ├── /          → Dashboard
│   ├── /properties → Properties CRUD
│   ├── /floors    → Floors CRUD
│   ├── /rooms     → Rooms CRUD
│   └── ...
└── /me/*          → ProtectedRoute (Tenant) → LayoutPortal
    └── (coming soon)
```

### Phase D2 — Khi Tenant Portal đủ Features (Multi-Space Architecture)

Khi Phase 3-4 (Contract self-service, Tickets, Invoices cho Tenant) hoàn tất, nâng lên kiến trúc **4-space** hoàn chỉnh:

```
App Router:
├── /auth/*        → LayoutAuth (public)
├── /manage/*      → LayoutManage (Owner, Manager, Staff)
│   ├── Sidebar đầy đủ: Properties, Floors, Rooms, Contracts, Invoices...
│   ├── Header: Org selector, notifications
│   └── Desktop-first layout
├── /me/*          → LayoutPortal (Tenant only)
│   ├── Bottom nav: Home, Phòng của tôi, Hợp đồng, Hóa đơn, Ticket
│   ├── Mobile-first, simplified UI
│   └── Scoping: chỉ thấy dữ liệu của phòng mình (membership-based)
└── /admin/*       → LayoutSysAdmin (System Admin super-user — tương lai)
    └── Quản lý Org, User toàn hệ thống
```

**Điều kiện để nâng lên Phase D2:**
- [ ] Tenant portal có ít nhất 3 features hoạt động (`/me/room`, `/me/contracts`, `/me/invoices`)
- [ ] `RBAC` Policy đã phân biệt rõ Tenant-scoped vs Org-scoped API
- [ ] `LayoutPortal` có Bottom Navigation hoàn chỉnh
