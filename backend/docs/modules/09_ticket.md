...
- [ ] **Gửi yêu cầu hỗ trợ (Tenant App)**:
    - Form nhập: Tiêu đề, Nội dung, Loại sự cố, Mức độ ưu tiên.
    - Upload ảnh/video hiện trường sự cố (Media).
- [ ] **Quản lý phiếu (Admin/Manager)**:
    - Danh sách phiếu: `OPEN`, `IN_PROGRESS`, `RESOLVED`, `CLOSED`.
    - Phân công nhân sự (`Staff`) xử lý.
    - Cập nhật tiến độ xử lý và ghi chú kỹ thuật.
- [ ] **Quản lý chi phí & Phụ tùng**:
    - Nhập chi phí vật tư, nhân công phát sinh.
    - Đính kèm hóa đơn mua hàng (nếu có).
    - Xuất dữ liệu chi phí vào hóa đơn thuê phòng (nếu là lỗi do khách).
...
|-----------|------|-------|
| `search` | string | Tìm theo tiêu đề, mã phiếu |
| `filter[status]` | string | `OPEN`, `IN_PROGRESS`, `RESOLVED`, `CLOSED` |
| `filter[priority]` | string | `LOW`, `MEDIUM`, `HIGH`, `URGENT` |
| `filter[property_id]` | uuid | Lọc theo tòa nhà |
| `filter[staff_id]` | uuid | Lọc phiếu được phân cho nhân viên |
| `sort` | string | `priority`, `created_at`, `status` |
...
```json
{
  ...
  "description": "Vòi nước bị rò rỉ mạnh từ sáng nay",
  "category": "PLUMBING",
  "priority": "HIGH",
  "room_id": "...",
...
|------|--------------------|---------|
| **Owner** | View & Delete | Quản lý tổng thể và chi phí |
| **Manager** | Assign & Update | Phân phối việc cho Staff |
| **Staff** | Update Progress | Chỉ cập nhật phiếu được phân công |
| **Tenant** | Create & View Own | Chỉ quản lý phiếu của phòng mình |

---

## Module Feature Matrix

| Feature | `tickets.index` | `tickets.costs.index` |
|---------|-----------------|-----------------------|
| Searching | ✅ | ❌ |
| Filtering | ✅ | ✅ |
| Sorting | ✅ | ✅ |
