 ...
 * @property-read \App\Features\Contract\Models\Contract[] $contracts Danh sách hợp đồng
 * @property-read \App\Features\Meter\Models\Meter[] $meters Danh sách đồng hồ
 * @property-read \App\Features\Invoice\Models\Invoice[] $invoices Danh sách hóa đơn
 */
        ...
        return [
            ...
            'contracts' => $this->whenLoaded('contracts', function () {
                return collect($this->contracts)->sortByDesc('start_date')->take(5)->values()->map(fn($contract) => [
                    ...
                    'status' => $contract->status ? strtolower($contract->status instanceof \BackedEnum ? $contract->status->value : (string) $contract->status) : null,
            ...
            'invoices' => $this->whenLoaded('invoices', function () {
                return collect($this->invoices)->sortByDesc('period_start')->take(10)->values()->map(fn($invoice) => [
                    ...
                    'status' => $invoice->status ? strtolower($invoice->status instanceof \BackedEnum ? $invoice->status->value : (string) $invoice->status) : null,
